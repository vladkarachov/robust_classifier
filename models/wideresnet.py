...
class NetworkBlock(nn.Module):
    ...
    def _make_layer(self, block, in_planes, out_planes, nb_layers, stride, activation, dropRate):
        ...
        for i in range(int(nb_layers)):
            layers.append(
                block(i == 0 and in_planes or out_planes, out_planes, i == 0 and stride or 1, activation, dropRate))
...
class WideResNet(nn.Module):
    def __init__(self, activation=nn.ReLU(inplace=True), depth=34, num_classes=10, widen_factor=10, dropRate=0.0):
        ...
        # 1st sub-block
        self.sub_block1 = NetworkBlock(n, nChannels[0], nChannels[1], block, 1, activation, dropRate)
        # 2nd block
